...
coverage:
	...
	PYTHONPATH=src $(PYTEST) --cov=su2_mcp --cov-report=term --cov-report=json:artifacts/coverage/coverage.json -q
...
clean:
	rm -rf .coverage .mypy_cache .pytest_cache .ruff_cache artifacts build dist docs/_build src/*.egg-info src/su2_mcp.egg-info
