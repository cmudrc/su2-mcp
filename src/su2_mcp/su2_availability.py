...
def discover_su2_binaries(
    candidates: Sequence[str] | None = None,
) -> list[SU2BinaryStatus]:
    ...
    probe_targets: Iterable[str] = (
        candidates if candidates is not None else DEFAULT_BINARIES
    )
    return [
        SU2BinaryStatus(name=target, path=shutil.which(target))
