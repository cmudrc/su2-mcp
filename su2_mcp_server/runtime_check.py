...
DOCKER_CMD = (
    "docker build -t tigl-mcp:dev . -f /path/to/tigl-mcp/Dockerfile\n"
    "  # Then run SU2 inside the container:\n"
    "  docker run --rm -v $PWD:/work tigl-mcp:dev bash -lc 'cd /work && SU2_CFD euler.cfg'"
)
...
To install, choose one of the following options:
  ...
  3. Docker (includes SU2 + Gmsh + TiGL):
     {DOCKER_CMD}

  4. Pre-built binaries from SU2 Foundation:
     {DOWNLOAD_URL}

  5. Install script (Linux/macOS):
