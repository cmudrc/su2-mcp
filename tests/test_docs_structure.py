"""Repository checks for docs scaffolding and generated example indexes."""
...
def _run_script(relative_path: str, *args: str) -> subprocess.CompletedProcess[str]:
    """Run a repository script from the repository root."""
    env = dict(os.environ)
...
def test_required_docs_files_exist() -> None:
    """The Sphinx docs entrypoints are present in the repository."""
    required_paths = [
...
def test_generated_example_docs_are_current() -> None:
    """The generated examples fragment matches checked-in example scripts."""
